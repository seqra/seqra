...
class InnerTypesTest : BaseTypesTest() {
    ...
    fun `inner classes linked to method`() {
        val classWithInners = findType<org.opentaint.opentaint-ir.impl.types.InnerClasses<*>>()
        val inners = classWithInners.innerTypes
        assertEquals(4, inners.size)
        val methodLinked = inners.first { it.typeName == "org.opentaint.opentaint-ir.impl.types.InnerClasses<W>.InnerClasses\$1" }
    ...
    fun `get not parameterized inner types`() {
        val innerClasses = findType<org.opentaint.opentaint-ir.impl.types.InnerClasses<*>>().innerTypes
    ...
    private fun field(fieldName: String): JIRClassType {
        return findType<org.opentaint.opentaint-ir.impl.types.InnerClasses<*>>().fields.first {
            it.name == fieldName
