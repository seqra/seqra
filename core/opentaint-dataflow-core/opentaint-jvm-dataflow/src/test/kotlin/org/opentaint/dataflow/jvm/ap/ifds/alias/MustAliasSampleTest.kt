...
class MustAliasSampleTest : BasicTestUtils() {
    override fun JIRLocalAliasAnalysis.getAliases(
        base: AccessPathBase.LocalVar,
        statement: JIRInst
    ): List<JIRLocalAliasAnalysis.AliasInfo>? = findMustAlias(base, statement)
    ...
    fun `test simple aliasing`() {
        ...
        assertFalse { apAliases.any { it.isPlainBase(Argument(1)) } }
    }
    ...
    @Test
    fun `test node next loop - loop diverges field chain`() {
        ...
        assertFalse { apAliases.any { it.isPlainBase(Argument(0)) } }
        assertFalse { apAliases.any { it.base == Argument(0) && it.accessors.isNotEmpty() } }
    }

    @Test
    fun `test node next loop data - loop diverges field chain`() {
    ...
    @Test
    fun `test node traversal - loop diverges field chain`() {
    ...
    @Test
    fun `test node traversal data - loop diverges field chain`() {
    ...
    fun `test conditional field write on argument receiver`() {
        ...
        assertFalse { apAliases.any { it.isPlainBase(Argument(1)) } }
        assertFalse { apAliases.any { it.isPlainBase(Argument(2)) } }
        assertTrue {
            apAliases.any {
                it.base == Argument(0) && it.accessors.singleFieldNamed(FIELD_VALUE)
            }
