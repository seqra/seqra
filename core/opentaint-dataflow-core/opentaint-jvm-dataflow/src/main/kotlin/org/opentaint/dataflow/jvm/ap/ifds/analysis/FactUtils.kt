...
object FactUtils {
    /**
     * @return Pair of lists, where left one has everything relevant to `suffix`, and right one has everything else.
     */
    fun splitBySuffix(suffix: List<Accessor>, fact: FinalFactAp): Pair<List<FinalFactAp>, List<FinalFactAp>> {
        ...
        if (tail.isEmpty()) {
            if (fact.startsWithAccessor(AnyAccessor)) {
                ...
                val suffixAfterAny = withHeadAfterAny?.prependAccessor(head)?.prependAccessor(AnyAccessor)

                val factWithoutAny = fact.clearAccessor(AnyAccessor)
                val remainingWithoutAny = factWithoutAny?.clearAccessor(head)
                val suffixWithoutAny = factWithoutAny?.readAccessor(head)?.prependAccessor(head)

                val suffixFact = listOfNotNull(suffixAfterAny, suffixWithoutAny)
                val remainingFact = listOfNotNull(remainingAfterAny, remainingWithoutAny)

                return suffixFact to remainingFact
            }
            ...
            val suffixFact = fact.readAccessor(head)?.prependAccessor(head)

            return listOfNotNull(suffixFact) to remainingFact
        }
        ...
        val (tailSuffix, tailRemaining) = splitBySuffix(tail, child)
        val suffixRelated = tailSuffix.map { it.prependAccessor(head) }
        val remaining = headRemaining + tailRemaining.map { it.prependAccessor(head) }

        return suffixRelated to remaining
    }
    ...
    private fun splitByMustAlias(
        ...
        return splitBySuffix(aliasAccessors, fact)
    }
