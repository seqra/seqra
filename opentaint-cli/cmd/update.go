...
Only upgrades are supported — downgrading to an older version is refused.`,
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		case utils.InstallMethodHomebrew:
			logrus.Info("opentaint was installed via Homebrew.")
			logrus.Info("Run: brew upgrade --cask opentaint")
			return
		case utils.InstallMethodGoInstall:
			logrus.Info("opentaint was installed via go install.")
			logrus.Info("Run: go install github.com/seqra/opentaint/v2@latest")
			return
		}
		...
		if cmp == 0 {
			logrus.Infof("opentaint is already up to date (v%s).", currentVersion)
			return
		}
		...
		if updateCheck {
			...
			printer.AddNode("")
			printer.AddNode("Run 'opentaint update' to update.")
			printer.Print()
			return
		}

		logrus.Info(formatters.FormatTreeHeader("OpenTaint Update"))
