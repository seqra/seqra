...
import org.opentaint.dataflow.jvm.ap.ifds.AnalysisRunner
import org.opentaint.dataflow.jvm.ap.ifds.CallPositionToAccessPathResolver
...
import org.opentaint.dataflow.jvm.ap.ifds.FactAwareConditionEvaluator
import org.opentaint.dataflow.jvm.ap.ifds.FactTypeChecker
import org.opentaint.dataflow.jvm.ap.ifds.FinalFactReader
import org.opentaint.dataflow.jvm.ap.ifds.InitialFactReader
...
import org.opentaint.dataflow.jvm.ap.ifds.MethodSummaryEdgeApplicationUtils
import org.opentaint.dataflow.jvm.ap.ifds.MethodSummaryEdgeApplicationUtils.SummaryEdgeApplication.SummaryApRefinement
import org.opentaint.dataflow.jvm.ap.ifds.MethodSummaryEdgeApplicationUtils.SummaryEdgeApplication.SummaryExclusionRefinement
import org.opentaint.dataflow.jvm.ap.ifds.MethodWithContext
...
import org.opentaint.dataflow.jvm.ap.ifds.TaintPassActionPreconditionEvaluator
import org.opentaint.dataflow.jvm.ap.ifds.TaintRulesProvider
import org.opentaint.dataflow.jvm.ap.ifds.TaintSourceActionPreconditionEvaluator
...
import org.opentaint.dataflow.jvm.ap.ifds.factCanBeModifiedByMethodCall
import org.opentaint.dataflow.jvm.ap.ifds.mapMethodCallToStartFlowFact
import org.opentaint.dataflow.jvm.ap.ifds.mapMethodExitToReturnFlowFact
...
class MethodTraceResolver(
    ...
    private val traits by lazy { JIRTraits(graph.cp) }
    ...
    sealed interface TraceEntry {
        ...
        sealed interface CallTraceEntry : TraceEntry {
            val method: JIRMethod
                get() = statement.callExpr?.method?.method ?: error("Not a call expr")
        }

        data class Final(override val fact: InitialFactAp, override val statement: JIRInst) : TraceEntry

        data class Sequential(override val fact: InitialFactAp, override val statement: JIRInst) : TraceEntry
    ...
    fun resolveIntraProceduralTrace(statement: JIRInst, fact: InitialFactAp): List<SummaryTrace> {
    ...
    fun resolveIntraProceduralTraceFromCall(
        ...
        val fact = mapMethodExitToReturnFlowFact(
    ...
    private fun TraceBuilder.processTraceEntry(entry: TraceEntry, initialFact: InitialFactAp?) {
        ...
        for (predecessor in graph.predecessors(entry.statement)) {
            val predecessorCall = predecessor.callExpr
            if (predecessorCall != null) {
                val returnValue: JIRImmediate? = (predecessor as? JIRAssignInst)?.lhv?.let {
                    it as? JIRImmediate ?: error("Non simple return value: $predecessor")
                }

                if (!factCanBeModifiedByMethodCall(returnValue, predecessorCall, entry.fact)) {
                ...
                val callees = runner.resolvedMethodCalls(methodEntryPoint, predecessorCall, predecessor)
                if (callees.isEmpty()) {
                ...
                val calleeWithExit = methodRelevantBases(predecessorCall, returnValue, callees, entry.fact)
                ...
                val preconditionFunction = MethodSequentPrecondition(predecessor)
    ...
    private fun TraceBuilder.applyCallRules(
        ...
        val predecessorFactsReaders = factsAtStatement.map { FinalFactReader(it, apManager) }

        val apResolver = CallPositionToAccessPathResolver(callExpr, returnValue)
        val jirValueResolver = CallPositionToJIRValueResolver(callExpr, returnValue)

        val ruleConditionEvaluator = FactAwareConditionEvaluator(
            traits, predecessorFactsReaders, apResolver, jirValueResolver
        )

        val entryFactReader = InitialFactReader(entry.fact, apManager)
        ...
        val passRulePreconditions = TaintConfigUtils.applyPassThrough(
            taintConfig,
            method,
        ...
        if (initialFact == null) {
            ...
            val sourceRulePreconditions = TaintConfigUtils.applySourceConfig(
                taintConfig,
                method,
                ruleConditionEvaluator,
    ...
    private fun methodRelevantBases(
        ...
        if (returnValue != null) {
            val returnValueBase = MethodFlowFunctionUtils.accessPathBase(returnValue) ?: return emptyList()
            if (returnValueBase == fact.base) {
                return methods.map { callee ->
                    val calleeExits = graph.exitPoints(callee.method).toList()
                    val exitBases = methodExitFactBases(calleeExits)
        ...
        mapMethodCallToStartFlowFact(callExpr.method.method, callExpr, fact) { _, calleeExitBase ->
    ...
    private fun methodStartFacts(
        ...
        if (!factCanBeModifiedByMethodCall(returnValue, callExpr, fact)) return emptyList()
    ...
    private fun factsAtStatement(statement: JIRInst, initialFact: InitialFactAp?): List<FinalFactAp> =
        if (initialFact == null) {
    ...
    private object DummyFactChecker : FactTypeChecker {
        override fun filterFactByLocalType(actualType: JIRType?, factAp: FinalFactAp): FinalFactAp = factAp
