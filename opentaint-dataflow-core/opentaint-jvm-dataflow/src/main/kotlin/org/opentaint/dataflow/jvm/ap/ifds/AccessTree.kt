...
class AccessTree(val base: AccessPathBase, val access: AccessNode, val exclusions: ExclusionSet) {
    ...
    class AccessNode private constructor(
        ...
        fun subtract(other: AccessNode): Pair<AccessNode?, AccessNode?> {
            if (this === other) return null to this

            val deltaIsAbstract = this.isAbstract && other.isAbstract
            val isAbstract = this.isAbstract && !other.isAbstract

            val deltaIsFinal = this.isFinal && other.isFinal
            val isFinal = this.isFinal && !other.isFinal

            val (elementAccess, elementDelta) = when {
                other.elementAccess == null -> elementAccess to null
                elementAccess == null -> null to null
                else -> elementAccess.subtract(other.elementAccess)
            }

            val fieldsDelta = arrayListOf<FieldAccessor>()
            val fieldNodesDelta = arrayListOf<AccessNode>()

            val transformedFields = transformFields { field, thisNode ->
                val otherNode = other.getFieldNode(field) ?: return@transformFields thisNode

                val (updatedNode, nodeDelta) = thisNode.subtract(otherNode)

                if (nodeDelta != null) {
                    fieldsDelta.add(field)
                    fieldNodesDelta.add(nodeDelta)
                }
            ...
            if (
                ...
                && isFinal == this.isFinal
                && elementAccess === this.elementAccess
                && transformedFields == null
            ) {
                return this to null
            }

            val delta = create(
                deltaIsAbstract, deltaIsFinal, elementDelta,
                fieldsDelta.toTypedArray(), fieldNodesDelta.toTypedArray()
            ).takeIf { !it.isEmpty }

            val fields = transformedFields?.first ?: fields
            val fieldNodes = transformedFields?.second ?: fieldNodes
            val thisNode = create(isAbstract, isFinal, elementAccess, fields, fieldNodes)
                .takeIf { !it.isEmpty }

            return thisNode to delta
        }

        fun intersect(other: AccessNode): AccessNode? {
            if (this === other) return this
            ...
            val elementAccess = when {
                this.elementAccess == null -> null
                other.elementAccess == null -> null
                else -> this.elementAccess.intersect(other.elementAccess)
            }

            val transformedFields = transformFields { field, thisNode ->
                val otherNode = other.getFieldNode(field) ?: return@transformFields null
                thisNode.intersect(otherNode)
            }
