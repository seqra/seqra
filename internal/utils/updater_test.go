...
func TestClassifyExePath(t *testing.T) {
	tests := []struct {
		...
		{
			"homebrew cellar",
			"/usr/local/Cellar/seqra/1.0/bin/seqra",
			"/home/user/go",
			InstallMethodHomebrew,
		},
		{
			"homebrew linuxbrew",
			"/home/user/.linuxbrew/Homebrew/bin/seqra",
			"/home/user/go",
			InstallMethodHomebrew,
		},
		{
...
func TestGetArchiveName(t *testing.T) {
	...
	want := fmt.Sprintf("seqra_%s_%s.%s", runtime.GOOS, runtime.GOARCH, ext)

	if name != want {
		t.Errorf("getArchiveName() = %q, want %q", name, want)
	}

	if !strings.HasPrefix(name, "seqra_") {
		t.Errorf("getArchiveName() should start with 'seqra_', got %q", name)
	}
}
...
func TestSelfUpdate(t *testing.T) {
	...
	archivePath := filepath.Join(archiveDir, "seqra_test.tar.gz")
	...
	existingBinary := filepath.Join(installDir, "seqra")
	...
	data, err := os.ReadFile(filepath.Join(installDir, "seqra"))
	if err != nil {
